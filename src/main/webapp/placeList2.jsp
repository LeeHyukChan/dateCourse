...
<body>
	...
	String location;
		if(name == null || name.equals(null) || name.equals("") || name ==""){
			 location = "matjibLogin.jsp";
		}else {
			location = "matjibLoginOk.jsp";
		}
	%>
...
<div class="box">
    <nav class="navbar" role="navigation" aria-label="main navigation">
        <div class="navbar-brand">
             <a class="navbar-item" href="<%= name == null ? location : location + "?name=" + name %>">
        현지인 추천 맛집탐방
    </a>
        </div>
    ...
    <script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=1390fe44c23adb51817af5b4c11dfc2b"></script>
<script>
        var map;
    ...
    <%
    List<PlaceVO> list = dao.festSelectAll(); 
...
%>
    var location<%= i %> = {
        ...
        title: '<%= list.get(i).getName().replace("'", "\\'") %>',
...
for (var i = 0; i < positions.length; i ++) {
        ...
        var infowindow = new kakao.maps.InfoWindow({
            content: '<p align=center ><br><img src ="images/'+positions[i].Gcode+'.jpeg"><br><a href = https://map.kakao.com/link/map/'+positions[i].title+","+positions[i].latlng.getLat()+','+positions[i].latlng.getLng()+'>'+positions[i].title+'</a></p>' // 인포윈도우에 표시할 내용
        });
